...
/* Pedido do usuário: PDF do extrato do Itaú (conta corrente/universitária), quando a pessoa não
   ...
   ignoradas, senão viram lançamentos falsos enormes). O valor pode ter sinal negativo (despesa)
   ou não (receita) — o próprio texto já indica isso. */
async function extrairTransacoesItauPdf(pdf) {
  ...
  for (const linha of todasAsLinhas) {
    ...
    if (resto.includes("SALDO DO DIA")) continue;
    ...
    transacoes.push({
      data: new Date(`${ano}-${mes}-${dia}T12:00:00`).toISOString(),
      descricao, valor: Math.abs(valor), tipo: valor < 0 ? "despesa" : "receita",
    });
  }
  return transacoes;
}
...
function TelaConfigFinancas({ categorias, setCategorias, contas, setContas, lancamentos, lancamentosFixos, limiar5Dias, setLimiar5Dias, onImportarExtrato, onConciliar, gruposOrcamento, setGruposOrcamento, rendaManual, setRendaManual, metas, cartoes, pin, onSalvarPin, onRemoverPin }) {
  ...
  return (
    <div className="h-full overflow-y-auto p-4">
      ...
      {modalImportar && (
        <ModalImportarExtrato contaFixa={modalImportar} contas={contas} lancamentosExistentes={lancamentos} categorias={categorias} onImportar={onImportarExtrato} onConciliar={onConciliar} onFechar={() => setModalImportar(null)} />
      )}
...
function TelaDocumentos({ documentos, setDocumentos, lancamentos, onSalvarLancamento, categorias, contas, arquivoCompartilhado, onUsarArquivoCompartilhado, onImportarExtrato, onConciliar }) {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {modalImportarExtrato && (
        <ModalImportarExtrato contaFixa={null} contas={contas} lancamentosExistentes={lancamentos} categorias={categorias} onImportar={onImportarExtrato} onConciliar={onConciliar} onFechar={() => setModalImportarExtrato(false)} />
      )}
      {confirmar && <ModalConfirmar titulo={confirmar.titulo} mensagem={confirmar.mensagem} textoConfirmar={confirmar.textoConfirmar} severo={confirmar.severo} onConfirmar={confirmar.acao} onCancelar={() => setConfirmar(null)} />}
    </div>
  );
...
function ModalImportarExtrato({ contaFixa, contas, lancamentosExistentes, categorias, onImportar, onConciliar, onFechar }) {
  useFecharComVoltar(true, onFechar);
  const [processando, setProcessando] = useState(false);
  const [erro, setErro] = useState(null);
  ...
  function processarConciliacao(transacoes, formato, contaEscolhidaId) {
    ...
    setResultado({
      ...
      duplicadas: transacoes.length - semReimportacao.length,
    });
  }
    ...
    try {
      ...
      if (ehOfx) {
        ...
        if (/DETALHE DOS MOVIMENTOS/i.test(textoPrimeiraPagina)) {
          ...
          transacoes = await extrairTransacoesItauPdf(pdf);
          formato = "pdf-itau";
        } else {
      ...
      if (contaId) processarConciliacao(transacoes, formato, contaId);
    } catch (err) {
      setErro(err.message);
    } finally {
      setProcessando(false);
  ...
  function escolherConta(id) {
    setContaId(id);
    processarConciliacao(transacoesBrutas, formatoDetectado, id);
  }
        ...
        {transacoesBrutas && !contaId && (
          <div className="mb-3">
            ...
            <div className="flex gap-2 flex-wrap">
              {contas.map((c) => <Chip key={c.id} selected={false} onClick={() => escolherConta(c.id)}>{c.nome}</Chip>)}
            </div>
          </div>
        )}
