...
/* Atalhos de lançamento rápido, baseados no histórico — pedido do usuário: "ajusta só o valor
   ...
   Testado com cenário onde algo muito frequente mas parado há 20 dias perde pra algo usado
   pouco mas muito recente — combina os dois sinais, não só um. */
function sugestoesRapidas(lancamentos, quantidade) {
  const grupos = {};
  const agora = Date.now();
  lancamentos.filter((l) => !l.previsto).forEach((l) => {
...
function ModalLancamento({ lancamento, tipoInicial, categorias, contas, contaPadraoId, cartoes, limiar5Dias, valorInicial, documentoId, lancamentos, onSalvar, onAdiar5Dias, onRemover, onAnexarDocumento, onFechar }) {
  useFecharComVoltar(true, onFechar);
  ...
  const ultimoLancamento = ehNovo && lancamentos?.length ? [...lancamentos].filter((l) => !l.previsto).sort((a, b) => new Date(b.data) - new Date(a.data))[0] : null;
  ...
  /* Atalho: preenche tudo a partir de um lançamento passado, foca e seleciona o valor pra só
     precisar ajustar o número (pedido do usuário: "ajusta só o valor e depois edita"). */
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-end justify-center z-[70]" onClick={onFechar}>
      <div className="bg-white rounded-t-2xl w-full max-w-md p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {ehNovo && (sugestoes.length > 0 || ultimoLancamento) && (
          <div className="mb-4">
            <label className="text-xs font-semibold text-stone-500 uppercase">Atalhos — ajusta só o valor</label>
            <div className="flex gap-2 overflow-x-auto mt-1 pb-1">
              {ultimoLancamento && (
                <button onClick={() => aplicarSugestao(ultimoLancamento)} className="shrink-0 flex flex-col items-start bg-emerald-50 border border-emerald-200 rounded-xl px-3 py-2 tap-target">
                  <span className="text-[10px] text-emerald-600 font-semibold">🔁 Repetir último</span>
                  <span className="text-xs text-stone-700 truncate max-w-[110px]">{ultimoLancamento.descricao}</span>
                  <span className="text-xs font-mono2 font-bold text-stone-800">{brl(ultimoLancamento.valor)}</span>
                </button>
              )}
              {sugestoes.filter((s) => s.id !== ultimoLancamento?.id).map((s) => {
                const cat = by(categorias, s.categoria_id);
                return (
                  <button key={s.id} onClick={() => aplicarSugestao(s)} className="shrink-0 flex flex-col items-start bg-stone-50 border border-stone-200 rounded-xl px-3 py-2 tap-target">
                    <span className="text-xs text-stone-700 truncate max-w-[110px]">{cat?.icone || "🏷️"} {s.descricao}</span>
                    <span className="text-xs font-mono2 font-bold text-stone-800">{brl(s.valor)}</span>
                  </button>
                );
              })}
            </div>
        ...
        <div className="flex items-center gap-2 border border-stone-300 rounded-xl px-3 py-2.5 mt-1 mb-3">
          <span className="text-stone-400 font-mono2">R$</span>
          <input ref={valorInputRef} value={valorTexto} onChange={(e) => setValorTexto(sanitizarEntradaPreco(e.target.value))} placeholder="ex: 150 = R$150,00" className="font-mono2 font-bold text-lg flex-1 outline-none" aria-label="Valor" />
        </div>
        ...
        {onAnexarDocumento && (
          <div className="mb-3">
            <label className="text-xs font-semibold text-stone-500 uppercase">Comprovante / nota fiscal</label>
            {documentoAnexadoId ? (
...
function TelaExtrato({ categorias, contas, lancamentos, documentos, onSalvarLancamento, onRemoverLancamento, lancamentosFixos, setLancamentosFixos, lembretes5Dias, limiar5Dias, onAdiar5Dias, onConfirmarLembrete, onDescartarLembrete, reflexoesMensais, onSalvarReflexao, metas, cartoes, gruposOrcamento, rendaManual, onResolverPendente, onAnexarDocumento, onFotografarRecibo, onAbrirConfig }) {
      ...
      <div className="p-4 pb-2 shrink-0">
        ...
        {subVisao === "lista" && (
          <div className="bg-white border border-stone-200 rounded-xl p-3 mb-2">
            ...
            <button onClick={() => setSaldoExpandido((v) => !v)} className="w-full flex items-center justify-between border-t border-stone-100 pt-2 tap-target">
              <span className="text-xs text-stone-700 font-semibold">Total de todas as contas{chaveMesEhFutura(chaveMes) ? " · projetado" : ""}</span>
              <div className="flex items-center gap-1.5">
                <span className="font-mono2 font-bold text-stone-800">{brl(saldoTotal)}</span>
                <span className="text-stone-400 text-[10px]">{saldoExpandido ? "▲" : "▼"}</span>
              </div>
            </button>
            {saldoExpandido && (
      ...
      {subVisao === "resumo" ? (
        ...
        <>
          ...
          <div className="p-4 pt-0 shrink-0 flex gap-2">
            <button onClick={() => { setTipoNovo("despesa"); setModalLancamento({}); }} className="flex-1 bg-emerald-700 text-white font-semibold py-3 rounded-xl tap-target">+ Novo lançamento</button>
            <button onClick={() => setModalFoto(true)} aria-label="Fotografar recibo" className="bg-emerald-100 text-emerald-700 text-xl px-4 rounded-xl tap-target">📸</button>
          </div>
        </>
      )}
...
function TelaDocumentos({ documentos, setDocumentos, lancamentos, onSalvarLancamento, categorias, contas }) {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {modalUpload && (
        <ModalUploadDocumento tipoDocumento={tipoParaNovoUpload} lancamentos={lancamentos} categorias={categorias} contas={contas} onSalvar={aoSalvarUpload} onFechar={() => setModalUpload(false)} />
      )}
      {modalContracheque && (
