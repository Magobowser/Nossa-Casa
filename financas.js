...
function gerarRelatorioTexto({ contas, lancamentos, lancamentosFixos, metas, cartoes, categorias, gruposOrcamento, rendaManual }) {
  const linhas = [];
  ...
  metas.forEach((m) => {
    linhas.push(`${m.nome}: ${brl(m.valor_guardado)} de ${brl(m.valor_alvo)}${m.prazo ? " — prazo " + new Date(m.prazo).toLocaleDateString("pt-BR") : ""}`);
  });
  linhas.push("");
  ...
  linhas.push("EXTRATO — ULTIMOS 3 MESES");
  let chave = chaveMesAtual();
  const chaves = [chave];
  for (let i = 0; i < 2; i++) { chave = mesAnteriorDe(chave); chaves.unshift(chave); }
  chaves.forEach((ch) => {
...
function ModalLancamento({ lancamento, tipoInicial, categorias, contas, contaPadraoId, cartoes, limiar5Dias, valorInicial, documentoId, onSalvar, onAdiar5Dias, onRemover, onAnexarDocumento, onFechar }) {
  useFecharComVoltar(true, onFechar);
    ...
    <div className="fixed inset-0 bg-black/40 flex items-end justify-center z-[70]" onClick={onFechar}>
      <div className="bg-white rounded-t-2xl w-full max-w-md p-5 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2 border border-stone-300 rounded-xl px-3 py-2.5 mt-1 mb-3">
          <span className="text-stone-400 font-mono2">R$</span>
          <input value={valorTexto} onChange={(e) => setValorTexto(sanitizarEntradaPreco(e.target.value))} placeholder="ex: 150 = R$150,00" className="font-mono2 font-bold text-lg flex-1 outline-none" aria-label="Valor" />
        </div>
...
function TelaExtrato({ categorias, contas, lancamentos, onSalvarLancamento, onRemoverLancamento, lancamentosFixos, setLancamentosFixos, lembretes5Dias, limiar5Dias, onAdiar5Dias, onConfirmarLembrete, onDescartarLembrete, reflexoesMensais, onSalvarReflexao, metas, cartoes, gruposOrcamento, rendaManual, onResolverPendente, onAnexarDocumento, onAbrirConfig }) {
        ...
        {subVisao === "lista" && (
          <div className="bg-white border border-stone-200 rounded-xl p-3 mb-2">
            ...
            <div className="border-t border-stone-100 pt-2 space-y-1.5">
              ...
              {contas.length > 1 && (
                <div className="flex items-center justify-between border-t border-stone-100 pt-1.5 mt-1.5">
                  <span className="text-xs text-stone-700 font-semibold">Total de todas as contas</span>
      ...
      {subVisao === "resumo" ? (
        ...
        <>
          <div className="flex-1 overflow-y-auto px-4 pb-24 space-y-2">
            ...
            {itensDoMes.map((item) => (
              <LinhaLancamento key={item.id} item={item} categoria={by(categorias, item.categoria_id)} nomeConta={contas.length > 1 ? by(contas, item.conta_id)?.nome : null} onAbrir={item.previsto ? confirmarPrevisto : setModalLancamento} />
            ))}
          </div>
...
function ModalRelatorio({ texto, onFechar }) {
  useFecharComVoltar(true, onFechar);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-end justify-center z-[80]" onClick={onFechar}>
      <div className="bg-white rounded-t-2xl w-full max-w-md p-5 max-h-[90vh] overflow-y-auto flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="text-xs text-stone-500 mb-3">Copia e cola numa IA (ChatGPT, Claude, etc.) pra pedir uma análise. De propósito só números e nomes, sem formatação chique.</p>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={copiar} className="flex-1 py-2.5 rounded-lg bg-emerald-700 text-white font-semibold tap-target">{copiado ? "✓ Copiado!" : "Copiar"}</button>
        </div>
      </div>
    </div>
  );
...
function TelaConfigFinancas({ categorias, setCategorias, contas, setContas, lancamentos, lancamentosFixos, limiar5Dias, setLimiar5Dias, onImportarExtrato, gruposOrcamento, setGruposOrcamento, rendaManual, setRendaManual, metas, cartoes }) {
  ...
  return (
    <div className="h-full overflow-y-auto p-4">
      ...
      {confirmar && <ModalConfirmar titulo={confirmar.titulo} mensagem={confirmar.mensagem} textoConfirmar={confirmar.textoConfirmar} severo={confirmar.severo} onConfirmar={confirmar.acao} onCancelar={() => setConfirmar(null)} />}
      {modalRelatorio && (
        <ModalRelatorio texto={gerarRelatorioTexto({ contas, lancamentos, lancamentosFixos, metas: metas || [], cartoes: cartoes || [], categorias, gruposOrcamento, rendaManual })} onFechar={() => setModalRelatorio(false)} />
      )}
      {modalImportar && (
...
function ModalMeta({ meta, onSalvar, onFechar }) {
  ...
  function salvar() {
    const valorAlvo = parseValorFinanceiro(valorAlvoTexto);
    ...
    onSalvar({ id: meta?.id || uid(), nome: nome.trim(), icone: icone.trim() || "🎯", valor_alvo: valorAlvo, valor_guardado: meta?.valor_guardado || 0, tipo, prazo: prazo ? new Date(prazo).toISOString() : null });
  }
